...
仓库提供的镜像支持将整个程序目录持久化到 `/var/www/html`。当挂载目录为空时，容器会自动复制程序文件、生成 `APP_KEY` 并创建上传目录。替换为新版本镜像后，容器会同步镜像中的程序代码，同时保留 `.env`、安装锁、上传文件和其他 `storage` 数据。
...
镜像支持 `WEB_PORT` 环境变量。旧版 1Panel 应用编排使用 `WEB_PORT=8089` 并将宿主端口映射到容器 `8089`，可以直接替换镜像地址，无需修改原有端口配置。
